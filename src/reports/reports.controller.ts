import { Controller, Get, Post, Patch, Delete, Body, Query, UseGuards } from '@nestjs/common';
import { ReportsService } from './reports.service';
import { CreateReportDto } from './dto/create-report.dto';
import { UpdateReportDto } from './dto/update-report.dto';
import { AuthGuard } from '../common/guards/auth.guard';
import { CurrentUser } from '../common/decorators/current-user.decorator';
import { PaginatedResponse } from '../common/types/api-response.type';

@Controller('v1/reports')
@UseGuards(AuthGuard)
export class ReportsController {
  constructor(private readonly reportsService: ReportsService) {}

  @Get()
  async findAll(
    @CurrentUser() user: { id: string },
    @Query('projectId') projectId: string,
    @Query('status') status: string,
    @Query('reportType') reportType: string,
    @Query('isFavorite') isFavorite: string,
    @Query('includeArchived') includeArchived: string,
    @Query('limit') limit: string,
    @Query('offset') offset: string,
  ): Promise<PaginatedResponse<unknown>> {
    const result = await this.reportsService.findAll(user.id, {
      projectId,
      status,
      reportType,
      isFavorite,
      includeArchived: includeArchived === 'true',
      limit: limit ? parseInt(limit) : undefined,
      offset: offset ? parseInt(offset) : undefined,
    });

    return {
      success: true,
      data: result.data,
      pagination: result.pagination,
    };
  }

  @Post()
  async create(@CurrentUser() user: { id: string }, @Body() dto: CreateReportDto) {
    const data = await this.reportsService.create(user.id, dto);
    return { success: true, data };
  }

  @Patch()
  async update(@CurrentUser() user: { id: string }, @Body() dto: UpdateReportDto) {
    const data = await this.reportsService.update(user.id, dto);
    return { success: true, data };
  }

  @Delete()
  async delete(@CurrentUser() user: { id: string }, @Query('id') id: string) {
    if (!id) {
      return { success: false, error: 'Report ID is required' };
    }
    const data = await this.reportsService.delete(user.id, id);
    return { success: true, ...data };
  }
}
